	...
	public static class PrefixOrder implements Comparator<Term> {
		...
		public int compare(Term v, Term w) {
			if(r > v.myWord.length() || r > w.myWord.length())  {
				return v.compareTo(w); // makes sure comparing normally when one is less than r
			}
			else {
			String vString = v.myWord;
			String wString = w.myWord;
			String newV = vString.substring(0, r);
			String newW = wString.substring(0, r);
